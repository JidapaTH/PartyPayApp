...
public class EventHelper {
    ...
    public int addEvent(String name, int status,int type,int pool, ArrayList<Member> member) {
        ...
        if(pool==1){
            member.add(0,new Member(0,"pooled fund",2));
            for(int i = 0; i<member.size();i++){
                member.get(i).change_cc(i);
            }
        } else{
    ...
    public boolean editEvent(int id, String name, int status,int type, ArrayList<Member> member,boolean pool) {
